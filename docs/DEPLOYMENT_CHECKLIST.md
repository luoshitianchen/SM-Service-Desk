# 服务台服务 上线检查清单（DEPLOYMENT_CHECKLIST）

> 服务：sm-service-desk（端口 8027）
...
2. `helm upgrade --install sm-service-desk helm/sm-service-desk -n sm-prod -f values-prod.yaml`。
...
- 步骤：`helm rollback sm-service-desk <上一 revision>`，或 ArgoCD 回退。
- 验证：指标恢复 + 冒烟通过。
